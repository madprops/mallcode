module.exports = (App) => {
  ...
  App.on_up = (ws, data, z_state) => {
    ...
    if (z_state.press_start_time) {
      ...
      if (typeof data.script === `string`) {
        z_state.current_script = data.script
      }

      let msg_up = JSON.stringify({type: `UP`, username: ws.username, sequence: z_state.current_sequence, script: z_state.current_script || Shared.default_script})
  ...
  App.on_client_letter = (ws, data, z_state) => {
    ...
    if (z_state.current_sequence) {
      ...
      let letter = App.shared.get_letter(z_state.current_script, z_state.current_sequence)
